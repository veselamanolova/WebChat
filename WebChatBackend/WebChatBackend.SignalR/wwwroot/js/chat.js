...
connection.on("ReceiveGlobalMessage", function (user, message) {
    var msg = message.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
    var encodedMsg = user + " from global group says " + msg;
    console.log(msg);
...
document.getElementById("sendGlobalMessageButton").addEventListener("click", function (event) {
    ...
    var message = document.getElementById("messageInput").value;
    console.log(message);
    connection.invoke("SendMessageToGlobalGroup", user, message).catch(function (err) {
        return console.error(err.toString());
    });
    document.getElementById("messageInput").value = "";    
    event.preventDefault();
